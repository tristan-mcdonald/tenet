function scale_ratio (current_viewport_width, minimum_value, maximum_value) {
    /**
     * Store the minimum viewport width at which `--ratio` custom property will be set. this must
     * match the values set in `breakpoints.styl` & `typography.styl`.
     */
    const min_range = 480;
    /**
     * Store the maximum viewport width at which `--ratio` custom property will be set. this must
     * match values set in `breakpoints.styl` & `typography.styl`.
     */
    const max_range = 1680;
    /**
     * Calculate and store the rate at wich our ratio changes when the viewport width changes by
     * 1px.
     */
    ...
    /**
     * Use `interpolated_ratio` value if it's within range. If it would be smaller than the smallest
     * acceptable ratio, use the smallest.
